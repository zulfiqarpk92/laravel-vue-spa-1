        ...
        >
          ...
          <td>
            <v-text-field
              ...
              type="number"
            />
            <span v-else>{{ item.salePrice | currency }}</span>
          </td>
          <td>
            <v-text-field
              v-if="edit"
          ...
          <td>{{ parseFloat(item.salePrice * item.quantity) | currency }}</td>
        </tr>
      </tbody>
      <tfoot v-if="edit">
        <tr>
          ...
          <th>{{ invoiceItems?.reduce((sum, item) => sum + parseFloat(item.salePrice * item.quantity), 0) ?? 0 | currency }}</th>
        </tr>
      </tfoot>
    </template>
  </v-simple-table>
